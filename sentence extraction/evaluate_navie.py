...
from data_loader_navie import DataLoader
import utils
from model_naive import newBertForTokenClassification
...
parser.add_argument('--model_dir', default='experiments/acl/scibert', help="Directory containing params.json")
...
parser.add_argument('--dataset', default="acl", help="semeval or acl")
parser.add_argument('--aug_type', default="wordEmbed", help="backTrans contextWordEmbed FEs keywords random tfidf wordEmbed")
...
def evaluate(model, data_iterator, params, epoch, mark='Eval', verbose=False, fw_squence=None):
    ...
    for round in range(params.eval_steps):
        ...
        batch_output, attention_output = model(batch_data,
        ...
        #attention_new = []
        #for batch in attention_output:
            #print ('batch',batch)
        #    values = ""
        #    for value in batch:
        #        values = values+str(value[0])+" "
            #print (values)
        #    attention_new.append(values)

        attention_outputs.extend([batch for batch in attention_output])
        pred_matrix.extend([str(batch) for batch in batch_output])
        pred_tags.extend(['O' if idx2tag.get(idx) is None else idx2tag.get(idx) for idx in np.argmax(batch_output, axis=1)])
...
if __name__ == '__main__':
    ...
    # Load data
    test_data = data_loader.load_data(params.dataset+'/scibert/tenfold_new/'+params.aug_type+'/scibert/train'+params.num)
    ...
    utils.load_checkpoint(os.path.join(args.model_dir, ""+params.num+"/"+args.restore_file + '.pth.tar'), model)
    ...
    fw_squence=open(args.result_dir+"/"+params.dataset,"w")
    epoch=0
    test_metrics, pred_tags, pred_matrix = evaluate(model, test_data_iterator, params, epoch, mark='Test', verbose=True)
