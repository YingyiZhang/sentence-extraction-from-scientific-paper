...
from model_bilstm_trans import newBertForTokenClassification
from data_loader_navie import DataLoader
from evaluate_navie import evaluate
import utils
...
parser.add_argument('--langModel', default='bert')
parser.add_argument('--model_dir',
                    default='experiments/semeval/scibert',#bart original
...
parser.add_argument('--aug', default=None, help="augmenter")
parser.add_argument('--save_model', default=False, help='if need to save the model')
...
def train_and_evaluate(model, train_data, val_data, optimizer, scheduler, params, model_dir, data_dir,
    ...
    if restore_file is not None:
        ...
        # utils.load_checkpoint(restore_path, model, None)
        ...
        if len(val_metrics) > 0:
            ...
            if params.save_model:
                utils.save_checkpoint({'epoch': epoch + 1,
                                  ...
                                  checkpoint=model_dir+"/"+str(params.num))
            if improve_f1 > 0:
...
if __name__ == '__main__':
    ...
    if params.aug is not None:
        train_data = data_loader.load_data(params.dataset+'/scibert/tenfold_new/combineAug/trainWithTrue_naive_'+params.aug+params.num, category='train',
                                             )
    else:
        train_data = data_loader.load_data(
            params.dataset + '/scibert/train' + params.num, category='train')

    val_data = data_loader.load_data(params.dataset+'/scibert/test'+params.num)
