...
class DataLoader(object):
    def __init__(self, data_dir, bert_model_dir, params, token_pad_idx=0):
        ...
        #self.length_pad_idx = self.length2idx[0]
    ...
    def readFile(self, file):
        ...
        for i, line in enumerate(line_list):
            ...
            if sentence_before=="start":
                tag_before = "O"
            else:
                tag_before = line_list[i-1].strip().split("\t")[-1]

            if sentence_end=="end":
                tag_end = "O"
            else:
                tag_end = line_list[i + 1].strip().split("\t")[-1]
            ...
            tag_before_id = self.tag2idx.get(tag_before)
            tag_end_id = self.tag2idx.get(tag_end)
            ...
            tags.append(tag_id)
            tags_before.append(tag_before_id)
            tags_end.append(tag_end_id)

        if self.isadjoin:
            return sentences, sentences_before, sentences_end, tags, tags_before, tags_end
        else:
    ...
    def load_sentences_tags(self, sentences_file, d, category='train'):
        ...
        with open(sentences_file, 'r', errors="ignore") as file:
            if self.isadjoin:
                sentences, sentences_before, sentences_end, tags, tags_before, tags_end = self.readFile(file)
            else:
    ...
    def data_iterator(self, data, shuffle=False, augClassi=None):
        ...
        for i in range(0, (data['size'] + self.batch_size - 1) // self.batch_size):
            ...
            if (i + 1) * self.batch_size < data['size']:
                ...
                if self.isadjoin:
                    ...
                    sentences_end = [data['data_end'][idx] for idx in
                                     order[i * self.batch_size:(i + 1) * self.batch_size]]
                    tags_before = [data['tags_before'][idx] for idx in order[i * self.batch_size:(i + 1) * self.batch_size]]
                    tags_end = [data['tags_end'][idx] for idx in
                                   order[i * self.batch_size:(i + 1) * self.batch_size]]
            else:
                if data['size'] - i * self.batch_size <= 1:
                    ...
                    if self.isadjoin:
                        ...
                        sentences_end = [data['data_end'][idx] for idx in
                                         order[i * self.batch_size:]]
                        tags_before = [data['tags_before'][idx] for idx in order[i * self.batch_size:]]
                        tags_end = [data['tags_end'][idx] for idx in order[i * self.batch_size:]]
                        sentences_before.extend([data['data_before'][idx] for idx in
                                            order[i * self.batch_size:]])
                        sentences_end.extend([data['data_end'][idx] for idx in
                                         order[i * self.batch_size:]])
                        tags_before.extend([data['tags_before'][idx] for idx in order[i * self.batch_size:]])
                        tags_end.extend([data['tags_end'][idx] for idx in order[i * self.batch_size:]])
            ...
            if self.isadjoin:
                batch_max_len_before = max([len(s) for s in sentences_before])
                max_len_before = min(batch_max_len_before, self.max_len)  # should change

                batch_max_len_end = max([len(s) for s in sentences_end])
                max_len_end = min(batch_max_len_end, self.max_len)  # should change
            ...
            if self.isadjoin:
                ...
                batch_tags_before = np.zeros((batch_len))
                batch_tags_end = np.zeros((batch_len))
            ...
            if self.isadjoin:
                ...
                batch_tags_before = torch.tensor(batch_tags_before, dtype=torch.long)
                batch_tags_end = torch.tensor(batch_tags_end, dtype=torch.long)
            ...
            if self.isadjoin:
                ...
                batch_lengths = \
                    batch_data.to(self.device), batch_data_before.to(self.device), batch_data_end.to(
                        self.device), \
                    batch_tags.to(self.device), batch_tags_before.to(self.device),batch_tags_end.to(self.device),\
                    batch_lengths.to(self.device)
            else:
                batch_data, batch_tags, batch_lengths = \
                    batch_data.to(self.device), batch_tags.to(self.device), batch_lengths.to(self.device)

            if self.isadjoin:
                yield batch_data, batch_data_before, batch_data_end, batch_tags, \
                      batch_tags_before, batch_tags_end, max_len, batch_lengths
            else:
