...
import utils
from model_transformer import newBertForTokenClassification
...
parser.add_argument('--bert_model_dir', default='allenai/scibert_scivocab_uncased',
...
parser.add_argument('--num', default="1", help="fold")
parser.add_argument('--dataset', default="semeval", help="semeval or acl")
parser.add_argument('--aug_type', default="null",
                    help="backTrans contextWordEmbed FEs keywords random tfidf wordEmbed")
...
def evaluate(model, data_iterator, params, epoch, mark='Eval', verbose=False, fw_squence=None):
    ...
    for round in range(params.eval_steps):
        ...
        if params.isadjoin:
            batch_data, batch_data_before, batch_data_end, batch_tags, max_len, batch_lengths = next(
                data_iterator)
        else:
        ...
        if params.isadjoin:
            #batch_output, attention_output = model(batch_data, data_before=batch_data_before, data_end=batch_data_end,
            #                                       token_type_ids=None, attention_mask=batch_masks, max_seq_len=max_len,
            #                                       input_lengths=batch_lengths, use_att=params.use_att,
            #                                       use_BiLSTM=params.use_BiLSTM)
            batch_output = model(batch_data, data_before=batch_data_before, data_end=batch_data_end,
                                                   token_type_ids=None, attention_mask=batch_masks, max_seq_len=max_len,
                                                   input_lengths=batch_lengths, use_att=params.use_att,
                                                   use_BiLSTM=params.use_BiLSTM)
            attention_output = batch_output
        else:
        ...
        attention_output = attention_output.detach().cpu().numpy()
        ...
        attention_outputs.extend([batch for batch in attention_output])
        pred_matrix.extend([str(batch) for batch in batch_output])
        pred_tags.extend(
            ['O' if idx2tag.get(idx) is None else idx2tag.get(idx) for idx in np.argmax(batch_output, axis=1)])
    ...
    return metrics, pred_tags, pred_matrix, attention_outputs
...
if __name__ == '__main__':
    ...
    test_data = data_loader.load_data(
        params.dataset + '/scibert/tenfold_new/scibert/temp')
    ...
    utils.load_checkpoint(os.path.join(args.model_dir, "adjoin" + params.num + "/" + args.restore_file + '.pth.tar'),
                          model)
    if args.fp16:
        model.half()
    ...
    fw_squence = open(
        args.result_dir + "/" + params.dataset, "w")
    fw_squence_att = open(
        args.result_dir + "/" + params.dataset+"att",
        "w")

    epoch = 0
    test_metrics, pred_tags, pred_matrix, attention_outputs = evaluate(model, test_data_iterator, params, epoch,
                                                                       mark='Eval', verbose=True)
